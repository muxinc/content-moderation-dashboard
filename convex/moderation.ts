...
export const listWithAssets = authenticatedQuery({
  args: {
    ...
    classificationFilter: v.optional(
      v.union(
        v.literal("pass"),
        v.literal("review"),
        v.literal("reject")
  ...
  handler: async (ctx, args) => {
    ...
    if (args.classificationFilter) {
      ...
      results = results.filter((r) => {
        ...
        if (!scores) return args.classificationFilter === "pass";
        const isReject =
          (sexualReject !== undefined && scores.sexual >= sexualReject) ||
          (violenceReject !== undefined && scores.violence >= violenceReject);
        const isReview =
          !isReject && (scores.sexual >= sexualReview || scores.violence >= violenceReview);
        if (args.classificationFilter === "reject") return isReject;
        if (args.classificationFilter === "review") return isReview;
...
export const counts = authenticatedQuery({
  ...
  handler: async (ctx) => {
    ...
    for (const r of all) {
      ...
      if (r.reviewStatus === "auto-approved") autoApproved++;
...
export const applyAutoActions = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (reviewScores) {
      const belowReview =
        reviewScores.sexual < (settings?.sexual.review ?? 0.9) &&
